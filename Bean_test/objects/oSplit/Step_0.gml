...
var BeanOrigW = global.BeanOrigW*0.75;
...
#region Merging back to Bean
if (merging) {
	grv = 0;
	x += (oBean.x - x) / 15;
	y += (oBean.y - y) / 15;
	if (point_distance(x, y, oBean.x, oBean.y) < (global.BeanOrigW*global.BeanSize/2-24)) {
		global.BeanSize += 1;
		merging = false;
		if (focus) {
			focus = false;
		...
		//if (!oAlert.display_finish) oAlert.display_finish = true;
		instance_destroy(oSplit);
	}
}
#endregion
...
if (global.controlable && !global.paused) {
	// drag starts
	if (drag == false and mouse_check_button(mb_left) and focus) {
		x_orig = mouse_x;
		y_orig = mouse_y;
		drag = true;
	}
